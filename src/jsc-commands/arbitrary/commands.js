...
const cloneObject = function(o) {
    const cloned = {};
    Object.keys(o)
        .filter(v => v != "hasStarted")
        .forEach(k => (cloned[k] = o[k]));
    return cloned;
};
...
const jscCommandsArray = function(gen, maxSize) {
    const shrinkImpl = function(arr) {
        ...
        const cuts = [];
        for (
            let start = Math.floor(xs.length / 2);
            start !== 0;
        ...
        return lazyseq
            .fromArray(cuts.map(v => xs.slice(v)))
    ...
    return jsc.bless({
        generator: size => {
            ...
            return [...new Array(arrsize).keys()].map(() =>
                gen.generator(size),
            );
        },
        shrink: arr => {
            ...
            return shrinkImpl(arr).map(narr => narr.map(cloneObject));
        },
        show: arr =>
            arr
                .map(
                    c =>
                        c.command.name !== undefined
                            ? c.command.name
                            : c.command.toString(),
                )
                .join(","),
