...
const padLeft = function(str, paddingValue) {
    return String(Array(paddingValue + 1).join(" ") + (str + [])).slice(
        -paddingValue,
    );
};
...
const printHead = function(name, record) {
    return (
        ...
        `${padMeasure("success")}  ${padMeasure("failed")}  ${padMeasure(
            "exception",
        )} |` +
        `${padMeasure("success")}  ${padMeasure("failed")}  ${padMeasure(
            "exception",
...
const prettyPrint = function(out) {
    ...
    const sep = head
        ...
        .replace(/[^|]/g, "-")
        .replace(/\|/g, "+");
    const content = Object.keys(out)
        .map(n => printOneRecord(n, out[n]))
        .join("\n");
    const footer = printOneRecord(
        "",
        Object.values(out).reduce(sumObjects, emptyOutput()),
    );
